...
public class SemaphoreImpl implements Semaphore {
    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1777getSegmentimpl(s$iv);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (SegmentOrClosed.m1779isClosedimpl(objFindSegmentInternal)) {
                j = andIncrement;
                z = false;
                z2 = true;
                break;
            }
            Segment segmentM1777getSegmentimpl = SegmentOrClosed.m1777getSegmentimpl(objFindSegmentInternal);
            while (true) {
                ...
                j = andIncrement;
                if (segment.id >= segmentM1777getSegmentimpl.id) {
                    z3 = true;
                    break;
                }
                if (!segmentM1777getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z3 = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, segmentM1777getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                    z3 = true;
                } else {
                    if (segmentM1777getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1777getSegmentimpl.remove();
                    }
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1777getSegmentimpl(objFindSegmentInternal);
