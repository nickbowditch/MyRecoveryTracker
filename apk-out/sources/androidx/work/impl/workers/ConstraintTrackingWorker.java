...
public final class ConstraintTrackingWorker extends ListenableWorker implements OnConstraintsStateChangedListener {
    ...
    private final void setupAndRunConstraintTrackingWork() {
        ...
        this.future.addListener(new Runnable() { // from class: androidx.work.impl.workers.ConstraintTrackingWorker$$ExternalSyntheticLambda1
            @Override // java.lang.Runnable
            public final void run() {
                ConstraintTrackingWorker.setupAndRunConstraintTrackingWork$lambda$1(job);
            }
        }, new SynchronousExecutor());
        if (workConstraintsTracker.areAllConstraintsMet(workSpec)) {
            logger.debug(ConstraintTrackingWorkerKt.TAG, "Constraints met for delegate " + className);
            try {
                ...
                innerFuture.addListener(new Runnable() { // from class: androidx.work.impl.workers.ConstraintTrackingWorker$$ExternalSyntheticLambda2
                    @Override // java.lang.Runnable
                    public final void run() {
                        ConstraintTrackingWorker.setupAndRunConstraintTrackingWork$lambda$3(this.f$0, innerFuture);
                    }
                }, getBackgroundExecutor());
                return;
            } catch (Throwable exception) {
                logger.debug(ConstraintTrackingWorkerKt.TAG, "Delegated worker " + className + " threw exception in startWork.", exception);
        ...
        logger.debug(ConstraintTrackingWorkerKt.TAG, "Constraints not met for delegate " + className + ". Requesting retry.");
