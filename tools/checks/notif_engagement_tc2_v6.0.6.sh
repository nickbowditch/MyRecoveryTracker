...
OUT_ABS="$FILES_DIR/daily_notification_engagement.csv"
IN_ABS="$FILES_DIR/notification_log.csv"
...
adb shell run-as "$PKG" sh -c "set -eu; mkdir -p '$FILES_DIR'; : > '$OUT_ABS'; cat > '$IN_ABS' <<'__CSV__'
ts,event,notif_id
$TS_Y_P,POSTED,tc2-a
$TS_Y_C,CLICKED,tc2-a
$TS_T_P,POSTED,tc2-b
$TS_T_C,CLICKED,tc2-b
__CSV__
...
i=0
while [ $i -lt 40 ]; do
  H="$(adb shell run-as "$PKG" head -n1 "$OUT_ABS" 2>/dev/null | tr -d '\r' || true)"
  if [ "$H" = "$EXP_HDR" ]; then
    RY="$(adb shell run-as "$PKG" awk -F, -v d="$YEST"  '$1==d{print $0}' "$OUT_ABS" 2>/dev/null || true)"
    RT="$(adb shell run-as "$PKG" awk -F, -v d="$TODAY" '$1==d{print $0}' "$OUT_ABS" 2>/dev/null || true)"
    [ -n "$RY" ] && [ -n "$RT" ] && break
  fi
  i=$((i+1)); sleep 1
done

HDR_OK=0; Y_OK=0; T_OK=0
HDR="$(adb shell run-as "$PKG" head -n1 "$OUT_ABS" 2>/dev/null | tr -d '\r' || true)"
...
DY=0; OY=0; DT=0; OT=0
if [ -n "$RY" ]; then DY="$(printf '%s\n' "$RY" | awk -F, '{print $3+0}')"; OY="$(printf '%s\n' "$RY" | awk -F, '{print $4+0}')"; [ "$DY" -ge 1 ] && [ "$OY" -ge 1 ] && Y_OK=1; fi
if [ -n "$RT" ]; then DT="$(printf '%s\n' "$RT" | awk -F, '{print $3+0}')"; OT="$(printf '%s\n' "$RT" | awk -F, '{print $4+0}')"; [ "$DT" -ge 1 ] && [ "$OT" -ge 1 ] && T_OK=1; fi
