...
OUT="evidence/v6.0/app_usage_by_category/ee3.txt"
RCV="$PKG/.TriggerReceiver"
A1="$PKG.ACTION_RUN_APP_CATEGORY_DAILY"
A2="$PKG.ACTION_RUN_APP_CATEGORY_ROLLUP"
mkdir -p "$(dirname "$OUT")"
...
($0 ~ p) && ($0 ~ /(App.*Category|Category.*Daily|AppUsage.*Category)/) && ($0 ~ /(SUCCEEDED|SUCCESS|COMPLETED|FINISHED)/){print; exit}' || true)"
[ -n "$WM_OK" ] && { PASS=1; break; }

LOG_OK="$(adb logcat -d 2>/dev/null | grep -iE "(App.*Category|Category.*Daily|AppUsage.*Category)" | grep -iE "enqueue|succeeded|success|completed|finished|done" || true)"
...
[ -n "$LOG_OK" ] && echo "$LOG_OK" || (adb logcat -d 2>/dev/null | grep -iE 'TriggerReceiver|(App.*Category|Category.*Daily|AppUsage.*Category)|WorkManager' || echo "[none]")
} | tee "$OUT" >/dev/null
