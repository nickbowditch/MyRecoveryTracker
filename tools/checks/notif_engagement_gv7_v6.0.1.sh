...
OUT="evidence/v6.0/notification_engagement/gv7.1.txt"
...
OK="$(
  ...
  NR==1 && $1=="date" { next }
  {
    ...
    if(rate!=""){
      ...
      exp=0
      if((del+0)>0){ exp=(op+0)/(del+0) }
      diff=(rate+0)-exp; if(diff<0) diff=-diff
      if(diff>0.02) { bad=1; exit }
    }
    if(d==t) ht=1
    if(d==y) hy=1
  }
  END{ if(!bad && ht && hy) print "OK"; else print "BAD" }
  '
)"
