...
adb exec-out run-as "$PKG" sh -c 'set -e; mkdir -p files/_tc2; for f in files/unlock_log.csv files/daily_unlocks.csv files/unlock_cursor.txt files/unlock_cursor state/unlock_cursor.txt files/unlock_rollup.state files/unlock_checkpoint.txt; do [ -f "$f" ] && mv "$f" "files/_tc2/$(basename "$f")"; done; printf "ts,event\n" > files/unlock_log.csv'
...
adb exec-out run-as "$PKG" sh -c 'set -e; for f in unlock_log.csv daily_unlocks.csv unlock_cursor.txt unlock_cursor unlock_rollup.state unlock_checkpoint.txt; do if [ -f "files/_tc2/$f" ]; then mv -f "files/_tc2/$f" "files/$f"; else rm -f "files/$f"; fi; done; rmdir files/_tc2 2>/dev/null || true'
