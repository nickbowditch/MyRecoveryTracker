...
PRE_ROW="$(adb exec-out run-as "$PKG" awk -F, -v d="$TODAY" 'NR>1&&$1==d{print;exit}' "$CSV" 2>/dev/null | tr -d $'\r' || true)"
echo "pre_row=${PRE_ROW:-<none>}"

adb shell cmd activity broadcast -n "$PKG/.TriggerReceiver" -a "$PKG.ACTION_RUN_MOVEMENT_INTENSITY" >/dev/null 2>&1 || true

POST_ROW="$(adb exec-out run-as "$PKG" awk -F, -v d="$TODAY" 'NR>1&&$1==d{print;exit}' "$CSV" 2>/dev/null | tr -d $'\r' || true)"
[ -n "$POST_ROW" ] || fail "no row for today"
echo "post_row=$POST_ROW"

echo "--- CSV tail ---"
adb exec-out run-as "$PKG" tail -n 5 "$CSV" 2>/dev/null | tr -d $'\r' || true

VAL="$(printf '%s\n' "$POST_ROW" | awk -F, '{print $2}')"
printf '%s\n' "$VAL" | grep -Eq '^[0-9]+$' || fail "intensity not integer"
awk -v c="$VAL" 'BEGIN{exit !(c>=0 && c<=10000)}' || fail "intensity out of bounds"
