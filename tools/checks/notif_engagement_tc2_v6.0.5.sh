...
OUT_REL="files/daily_notification_engagement.csv"
IN_REL="files/notification_log.csv"
...
i=0
while [ $i -lt 40 ]; do
  H="$(adb shell run-as "$PKG" sh -c 'head -n1 "'"$OUT_REL"'" 2>/dev/null' | tr -d '\r' || true)"
  if [ "$H" = "$EXP_HDR" ]; then
    RY="$(adb shell run-as "$PKG" sh -c 'awk -F, -v d="'"$YEST"'"  "$1==d{print \$0}" "'"$OUT_REL"'" 2>/dev/null' || true)"
    RT="$(adb shell run-as "$PKG" sh -c 'awk -F, -v d="'"$TODAY"'" "$1==d{print \$0}" "'"$OUT_REL"'" 2>/dev/null' || true)"
    [ -n "$RY" ] && [ -n "$RT" ] && break
  fi
  i=$((i+1))
  sleep 0.5
...
HDR="$(adb shell run-as "$PKG" sh -c 'head -n1 "'"$OUT_REL"'" 2>/dev/null' | tr -d '\r' || true)"
RY="$(adb shell run-as "$PKG" sh -c 'awk -F, -v d="'"$YEST"'"  "$1==d{print \$0}" "'"$OUT_REL"'" 2>/dev/null' || true)"
RT="$(adb shell run-as "$PKG" sh -c 'awk -F, -v d="'"$TODAY"'" "$1==d{print \$0}" "'"$OUT_REL"'" 2>/dev/null' || true)"

HDR_OK=0; Y_OK=0; T_OK=0; DY=0; OY=0; DT=0; OT=0
