...
T="${HC_TIMEOUT:-20}"
LOG="evidence/v6.0/unlocks/health_unlocks.15.txt"
export LC_ALL=C
...
ensure_device() {
  ...
  for i in {1..30}; do
    ...
    if [ "$st" = "device" ]; then adb shell true >/dev/null 2>&1 && return 0; fi
    sleep 1
  done
  return 1
}
...
latest_for_base() {
  ...
  base="$(basename "$1")"
  base="${base%%_v6.0*}"
  ls -1 "$dir/${base}"_v6.0*.sh 2>/dev/null | sort -V | tail -n1
}
