...
OUT="evidence/v6.0/sleep/di3.6.txt"
mkdir -p "$(dirname "$OUT")"
...
NR>1{
d=$di; st=$si; wt=$wi
vmin=(mi? $mi:""); vhr=(hi? $hi:"")

if(!date_ok(d)) { bad=1; exit }
if(!time_ok(st) || !time_ok(wt)) { bad=1; exit }

if(vmin!=""){
if(!is_int(vmin)) { bad=1; exit }
if((vmin+0)<0 || (vmin+0)>1440) { bad=1; exit }
} else if(vhr!=""){
if(!is_num(vhr)) { bad=1; exit }
if((vhr+0)<0 || (vhr+0)>24) { bad=1; exit }
}
}
END{ exit(bad?1:0) }
