...
OUT="evidence/v6.0/lnsu/tc2.2.txt"
...
[ -n "$D" ] || { echo "TC-2 RESULT=FAIL (no daily_lnsu.csv)" | tee "$OUT"; exit 4; }

DAILY_VAL="$(adb exec-out run-as "$PKG" awk -F, -v d="$D" 'NR>1&&$1==d{print $3;exit}' "$CSV_DAILY" 2>/dev/null | tr -d '\r')"
...
# pass 1: scan and accumulate, carrying last state before window start
while IFS=, read -r ts state; do
  ...
  if [ "$t" -lt "$ws" ]; then
    last_on="$cur_on"; prev_ts="$t"; prev_on="$cur_on"; continue
  fi
...
echo "TC-2 D=$D recomputed=$RECOMP daily=$DAILY_VAL" | tee "$OUT" >/dev/null
