...
PH="$(adb exec-out run-as "$APP" sed -n '1p' "$BYAPP" 2>/dev/null | tr -d '\r' || true)"
[ "$CH" = "date,category,minutes" ] || fail "(bad category header)"
[ "$PH" = "date,category,minutes,package" ] || fail "(bad per-app header)"
...
echo "date,category,minutes,package" >"$appf"
echo "'"$T"',social,30.0,com.test.app1" >>"$appf"
echo "'"$T"',social,15.0,com.test.app2" >>"$appf"
echo "'"$T"',productivity,45.0,com.test.app3" >>"$appf"
' >/dev/null 2>&1 || true
