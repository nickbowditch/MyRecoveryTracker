...
OUT="evidence/v6.0/notification_engagement/ee3.3.txt"
...
YDAY="$(adb shell toybox date -d "$TODAY -1 day" +%F 2>/dev/null | tr -d '\r' || echo "$TODAY")"
...
while [ $i -lt 30 ]; do
  ...
  if adb exec-out run-as "$PKG" sh -c '[ -f "'"$CSV"'" ] && tail -n 10 "'"$CSV"'"' 2>/dev/null \
     | tr -d '\r' | grep -qE "^($TODAY|$YDAY),"; then
    PASS=1; break
  fi
