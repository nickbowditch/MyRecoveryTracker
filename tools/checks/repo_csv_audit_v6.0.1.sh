...
for f in $FILES; do
  ...
  [ -n "$hdr" ] || { echo "BAD:EMPTY_HEADER $f" | tee -a "$OUT"; fail=$((fail+1)); continue; }
  cols="$(printf '%s\n' "$hdr" | awk -F, '{print NF}')"
  adb exec-out run-as "$PKG" awk -F, -v n="$cols" 'NR>1{ if(NF!=n){ exit 1 } }' "$f" >/dev/null 2>&1 || { echo "BAD:COLUMN_MISMATCH $f" | tee -a "$OUT"; fail=$((fail+1)); continue; }
  ...
  echo "OK:$f:$hdr" | tee -a "$OUT" >/dev/null
done
