...
[ -n "$TARGET" ] || fail "(no target date)"

RAW_N="$(adb exec-out run-as "$APP" awk -F, -v d="$TARGET" '
NR>1{
dd=$1; gsub(/^[[:space:]]+|[[:space:]]+$/,"",dd);
if(dd==d) n++
}
END{print n+0}' "$RAW" 2>/dev/null || echo 0)"

DAILY_N="$(adb exec-out run-as "$APP" awk -F, -v d="$TARGET" '
NR>1{
dd=$1; gsub(/^[[:space:]]+|[[:space:]]+$/,"",dd);
if(dd==d){ gsub(/^[[:space:]]+|[[:space:]]+$/,"",$2); print $2; exit }
}
' "$DAILY" 2>/dev/null | tr -d '\r' || true)"
[ -n "${DAILY_N:-}" ] || DAILY_N=0
