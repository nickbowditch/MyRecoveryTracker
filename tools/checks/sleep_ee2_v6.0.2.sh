...
before="$(adb exec-out run-as "$PKG" sh -c 'tr -d "\r" < "'"$CSV"'" 2>/dev/null | sha1sum 2>/dev/null | awk "{print \$1}"' || true)"
...
after="$(adb exec-out run-as "$PKG" sh -c 'tr -d "\r" < "'"$CSV"'" 2>/dev/null | sha1sum 2>/dev/null | awk "{print \$1}"' || true)"
