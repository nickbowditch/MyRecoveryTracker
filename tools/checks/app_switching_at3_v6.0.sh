#!/bin/sh
set -eu
OUT="evidence/v6.0/app_switching/at3.txt"
...
append_if_missing '!evidence/v6.0/app_switching'
append_if_missing '!evidence/v6.0/app_switching/*.csv'
append_if_missing '!evidence/v6.0/app_switching/*.txt'

git add .gitignore >/dev/null 2>&1 || true
git commit -m "AT3: whitelist evidence app_switching paths (v6.0)" >/dev/null 2>&1 || true
git rm -r --cached evidence >/dev/null 2>&1 || true

touch evidence/v6.0/app_switching/probe.txt
if git add evidence/v6.0/app_switching/probe.txt >/dev/null 2>&1; then
