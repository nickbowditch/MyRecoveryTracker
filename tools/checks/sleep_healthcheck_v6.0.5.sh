...
PASS_RE='\bRESULT[[:space:]]*[:=][[:space:]]*PASS\b'
FAIL_RE='\bRESULT[[:space:]]*[:=][[:space:]]*FAIL\b'
...
run_check() {
  ...
  if [ ! -x "$script" ]; then printf "%s: ⚠️  MISSING\n" "$chk" | tee -a "$OUT"; return; fi
  TMP="$(mktemp)"
  reset_adb
  "$script" >"$TMP" 2>&1 || true

  if grep -Eq "$PASS_RE" "$TMP"; then
    printf "%s: ✅ PASS\n" "$chk" | tee -a "$OUT"
  elif grep -Eq "$FAIL_RE" "$TMP"; then
    if [ "$chk" = "sleep_di3_v6.0.27.sh" ]; then
      sleep 1
      reset_adb
      : > "$TMP"
      "tools/checks/sleep_di3_v6.0.27.sh" >"$TMP" 2>&1 || true
      if grep -Eq "$PASS_RE" "$TMP"; then
        printf "%s: ✅ PASS (retry)\n" "$chk" | tee -a "$OUT"
      else
        printf "%s: ❌ FAIL\n" "$chk" | tee -a "$OUT"
