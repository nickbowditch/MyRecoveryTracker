...
OUT="evidence/v6.0/unlocks/tc1.3.txt"
mkdir -p "$(dirname "$OUT")"
...
printf '%s\n' "$CSV" | awk -F',' -v a="$D_LOCAL" -v b="$D_UTC" 'NR>1 && ($1==a || $1==b){f=1;exit} END{exit f?0:1}'
rc=$?
...
echo "TC-1 RESULT=FAIL (no row for $D_LOCAL or $D_UTC)" | tee "$OUT"
exit 1
fi
