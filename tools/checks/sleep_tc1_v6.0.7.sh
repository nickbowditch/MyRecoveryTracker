...
OUT="evidence/v6.0/sleep/tc1.7.txt"
CSV_PATH="files/daily_sleep.csv"
mkdir -p "$(dirname "$OUT")"
...
case "$HDR" in
  date,minutes|date,duration_minutes|date,duration_hours) : ;;
  *) echo "TC-1 RESULT=FAIL (bad header: $HDR)" | tee "$OUT"; exit 5 ;;
esac
