...
OUT="evidence/v6.0/notification_engagement/di3.1.txt"
mkdir -p "$(dirname "$OUT")"
...
function approx(a,b,tol){ return (a-b<tol && b-a<tol) }
{
d=$1; v=$2; del=$3; op=$4; rate=$5
if(!date_ok(d)) exit 1
...
if(rate!=""){
  ...
  exp=(del>0?op/del:0)
  if(!approx(rate,exp,0.01)) exit 1
}
}
