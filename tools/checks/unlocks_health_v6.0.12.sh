...
T="${HC_TIMEOUT:-20}"
LOG="evidence/v6.0/unlocks/health_unlocks.12.txt"
export LC_ALL=C
...
ts(){ date +"%F %T"; }
latest_for_base(){ ls -1 tools/checks/"${1}"_v6.0*.sh 2>/dev/null | sort -V | tail -n1; }
...
recap() {
  awk '
    /^[A-Z0-9-]+[[:space:]]+RESULT[:=][[:space:]](PASS|FAIL)/{
      key=$1
      if ($0 ~ /RESULT[:=][[:space:]]PASS/) v="PASS"; else v="FAIL"
      print key "=" v
    }
  ' "$1" >> "$SUM"
}
...
run_block() {
  ...
  for f in $(ls $pat 2>/dev/null | sort -V); do
    ...
    ensure_device || { echo "==> $(ts) SKIP $f (no device)"; continue; }
