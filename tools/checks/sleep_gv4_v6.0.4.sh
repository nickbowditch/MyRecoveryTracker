#!/bin/bash
OUT="evidence/v6.0/sleep/gv4.4.txt"

git rev-parse --is-inside-work-tree >/dev/null 2>&1 || { echo "GV-4 RESULT=FAIL (not a git repo)" | tee "$OUT"; exit 2; }

shopt -s nullglob
FILES=($(ls evidence/v6.0/sleep/*.txt 2>/dev/null | grep -E '/[a-z0-9_]+\.([0-9]+)\.txt$' || true))
[ ${#FILES[@]} -gt 0 ] || { echo "GV-4 RESULT=FAIL (no numbered PASS logs)" | tee "$OUT"; exit 3; }
