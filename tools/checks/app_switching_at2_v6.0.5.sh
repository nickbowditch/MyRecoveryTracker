...
deadline_ev=$(( $(date +%s) + 30 ))
have_today=0
while :; do
  CNT="$(adb exec-out run-as "$APP" awk -F, -v d="$T" 'NR>1 && $1==d && ($3=="FOREGROUND"||$3=="MOVE_TO_FOREGROUND"){c++} END{print c+0}' "$CSV_EVENTS" 2>/dev/null | tr -d '\r' || echo 0)"
  if [ "${CNT:-0}" -gt 0 ]; then have_today=1; break; fi
  [ "$(date +%s)" -ge "$deadline_ev" ] && break
  sleep 1
done
[ "$have_today" -eq 1 ] || {
  {
    echo "AT2 RESULT=FAIL (no today FG events in usage_events)"
    echo "--- DEBUG: HEAD usage_events ---"
    adb exec-out run-as "$APP" head -n 10 "$CSV_EVENTS" 2>/dev/null | tr -d '\r' || true
    echo "--- DEBUG: TODAY COUNT ---"
    echo "$CNT"
  } | tee "$OUT"
  exit 1
}
...

{
  ...
  echo
  echo "--- DEBUG: TODAY FG EVENT COUNT ---"
  adb exec-out run-as "$APP" awk -F, -v d="$T" 'NR>1 && $1==d && ($3=="FOREGROUND"||$3=="MOVE_TO_FOREGROUND"){c++} END{print c+0}' "$CSV_EVENTS" 2>/dev/null | tr -d '\r' || true
  echo "--- DEBUG: TAIL usage_events (today) ---"
  adb exec-out run-as "$APP" awk -F, -v d="$T" 'NR==1{print;next} $1==d' "$CSV_EVENTS" 2>/dev/null | tail -n 10 | tr -d '\r' || true
  echo "--- DEBUG: DAILY ROW ---"
