...
OUT="evidence/v6.0/notification_engagement/ee2.3.txt"
NAME="EngagementRollup"
RCV="$PKG/.TriggerReceiver"
ACT1="$PKG.ACTION_RUN_ENGAGEMENT_ROLLUP"
mkdir -p "$(dirname "$OUT")"
...
{blk=blk $0 ORS}
/RTC|READY|WAITING/{
if (blk ~ p && blk ~ /(Engagement|Notif|Rollup)/) {print blk; exit}
}' || true)"

WM="$(adb shell dumpsys activity service WorkManager 2>/dev/null \
| awk -v p="$PKG" 'BEGIN{IGNORECASE=1}
{line=$0}
(line ~ p) && (line ~ /(Engagement|Notif|Rollup)/){print line; found=1; exit}
END{if(!found) exit 0}' || true)"

LOG="$(adb logcat -d 2>/dev/null \
| grep -iE 'TriggerReceiver|WorkManager|enqueue|Engagement|Notif' || true)"
...
if [ -n "$JS" ] || [ -n "$WM" ] || printf '%s' "$LOG" | grep -qiE '(Engagement|Notif).*(enqueue|worker|rollup)'; then
