...
fail() {
  ...
  echo "--- DEBUG: WorkManager slice ---" | tee -a "$OUT"
  adb shell dumpsys activity service WorkManager 2>/dev/null | tr -d $'\r' | awk -v p="$PKG" 'BEGIN{IGNORECASE=1} ($0~p)&&($0~/Movement|Intensity|Worker|SUCCEEDED|ENQUEUED|RUNNING/){print}' | sed -n '1,160p' | tee -a "$OUT" || true
  echo "--- DEBUG: jobscheduler slice ---" | tee -a "$OUT"
  ...
  adb logcat -d -v brief 2>/dev/null | grep -Ei "$PKG|MovementIntensity|IntensityWorker|MovementIntensityDailyWorker|WorkManager|TriggerReceiver|enqueue|Succeeded|SUCCEEDED|Finished|COMPLETED" | tail -n 200 | tr -d $'\r' | tee -a "$OUT" || true
  exit 1
}
...
while (( SECONDS < deadline )); do
  ...
  FOUND_LOG="$(adb logcat -d -v brief 2>/dev/null | grep -Ei 'MovementIntensityDailyWorker|IntensityWorker|MovementIntensity.*(SUCCEEDED|Succeeded|success|Completed)|WorkManager.*(SUCCEEDED|Completed)|onStartJob.*Movement|Finished.*Movement' | tail -n 1 | tr -d $'\r' || true)"
  [ -n "$FOUND_LOG" ] && break
  sleep 1
done

{
  ...
  adb logcat -d -v brief 2>/dev/null | grep -Ei "$PKG|MovementIntensity|IntensityWorker|MovementIntensityDailyWorker|WorkManager|TriggerReceiver|enqueue|SUCCEEDED|Succeeded|Finished|Completed" | tail -n 200 | tr -d $'\r' || true
} | tee -a "$OUT" >/dev/null

if [ -n "$FOUND_CSV" ] || [ -n "$FOUND_LOG" ]; then
