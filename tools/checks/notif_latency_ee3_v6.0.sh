...
OUT="evidence/v6.0/notification_latency/ee3.txt"
RCV="$PKG/.TriggerReceiver"
A1="$PKG.ACTION_RUN_NOTIFICATION_LATENCY_ROLLUP"
A2="$PKG.ACTION_RUN_LATENCY_ROLLUP"
A3="$PKG.ACTION_RUN_NOTIF_LATENCY_ROLLUP"
mkdir -p "$(dirname "$OUT")"
...
($0 ~ p) && ($0 ~ /Latency/) && ($0 ~ /(SUCCEEDED|SUCCESS|COMPLETED|FINISHED)/){print; exit}' || true)"
[ -n "$WM_OK" ] && { PASS=1; break; }

LOG_OK="$(adb logcat -d 2>/dev/null | grep -i "Latency" | grep -i -E "enqueue|succeeded|success|completed|finished|done" || true)"
...
[ -n "$LOG_OK" ] && echo "$LOG_OK" || (adb logcat -d 2>/dev/null | grep -i -E 'TriggerReceiver|Latency|WorkManager' || echo "[none]")
} | tee "$OUT" >/dev/null
