...
adb exec-out run-as "$APP" cat files/daily_late_night_screen_usage.csv 2>/dev/null \
  > evidence/v6.0/lnsu/dlnsu.before.csv || : 
...
adb exec-out run-as "$APP" cat files/daily_late_night_screen_usage.csv \
  > evidence/v6.0/lnsu/dlnsu.after.csv
...
[ "${b:-none}" != "$a" ] && { echo "AT1 RESULT=PASS" | tee "$OUT"; exit 0; }
echo "AT1 RESULT=FAIL (no change detected)" | tee "$OUT"; exit 1
