...
sleep 1
adb shell logcat -d > /tmp/_usage_events_at1_log.txt 2>/dev/null || true
cat /tmp/_usage_events_at1_log.txt | tee "$LOG" >/dev/null

grep -q "TriggerReceiver.ACTION_RUN_USAGE_EVENTS_DAILY" "$LOG" || fail "(receiver not observed)"
grep -qi "UsageEvents" "$LOG" || fail "(worker not observed)"
grep -qiE "WM-WorkerWrapper.(SUCCEEDED|SUCCESS).*UsageEvents" "$LOG" || fail "(worker did not succeed)"
