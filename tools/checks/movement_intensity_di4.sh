...
exec > >(tee "$OUT") 2>&1
fail(){ echo "DI4 RESULT=FAIL ($1)"; exit 1; }

echo "started_at=$(date -u +'%Y-%m-%dT%H:%M:%SZ')"

adb get-state >/dev/null 2>&1 || { echo "DI4 RESULT=FAIL (no device)"; exit 2; }
adb shell pm path "$PKG" >/dev/null 2>&1 || { echo "DI4 RESULT=FAIL (app not installed)"; exit 3; }

git rev-parse --is-inside-work-tree >/dev/null 2>&1 || fail "not inside a git repository"
REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || echo "<unknown>")"
echo "repo_root=$REPO_ROOT"

SENT="$EVDIR/.di4_track_sentinel"
echo "di4-sentinel" > "$SENT"

set +e
CI_DIR_VERBOSE="$(git check-ignore -v "$EVDIR" 2>/dev/null)"
CI_FILE_VERBOSE="$(git check-ignore -v "$SENT" 2>/dev/null)"
git check-ignore -q "$EVDIR";  CI_DIR_STATUS=$?
git check-ignore -q "$SENT";   CI_FILE_STATUS=$?
GA_DRYRUN="$(git add -n "$SENT" 2>&1)"
STATUS_PORC="$(git status --porcelain --ignored "$SENT" 2>/dev/null)"
set -e
