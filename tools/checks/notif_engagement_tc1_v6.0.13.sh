...
OUT="evidence/v6.0/notification_engagement/tc1.13.txt"
mkdir -p "$(dirname "$OUT")"
...
echo "$t 10:00:00,POSTED,tc1-a"  >> "$raw"
echo "$t 10:01:00,POSTED,tc1-b"  >> "$raw"
echo "$t 10:05:00,CLICKED,tc1-a" >> "$raw"
IN

adb shell cmd activity broadcast -n "$RCV" -a "$ACT" --receiver-foreground --user 0 >/dev/null 2>&1 || true
sleep 2

row1="$(adb exec-out run-as "$PKG" awk -F, -v d="$T" 'NR>1&&$1==d{print;exit}' "$DAILY" | tr -d '\r')"
adb shell cmd activity broadcast -n "$RCV" -a "$ACT" --receiver-foreground --user 0 >/dev/null 2>&1 || true
sleep 1
row2="$(adb exec-out run-as "$PKG" awk -F, -v d="$T" 'NR>1&&$1==d{print;exit}' "$DAILY" | tr -d '\r')"

case "$row1" in
  "$T",1,2,1,0.500000) ;; 
  *) echo "TC1 RESULT=FAIL (row=$row1)" | tee "$OUT"; exit 1 ;;
esac

[ "$row1" = "$row2" ] && echo "TC1 RESULT=PASS" | tee "$OUT" || { echo "TC1 RESULT=FAIL (non-idempotent)" | tee "$OUT"; exit 1; }
