...
COUNTER_LINE="$(printf "%s\n" "$SENS" | awk '/Sensor List/{sl=1} sl && /(step[_ ]?counter|android\.sensor\.step_counter)/ {print; exit}' || true)"
DETECTOR_LINE="$(printf "%s\n" "$SENS" | awk '/Sensor List/{sl=1} sl && /(step[_ ]?detector|android\.sensor\.step_detector)/ {print; exit}' || true)"
...
{ printf "%s\n" "$PDUMP" | grep -E 'uses-feature' | grep -Ei 'step|sensor' || true; } || true
echo "--- SENSOR LINES ---"
printf "[counter] %s\n" "${COUNTER_LINE:-<none>}"
printf "[detector] %s\n" "${DETECTOR_LINE:-<none>}"
