...
OUT="evidence/v6.0/notification_latency/tc2.21.txt"
mkdir -p "$(dirname "$OUT")"
...
echo "$T 13:00:00,POSTED,tc2-today"  >> "\$raw"
echo "$T 13:05:00,CLICKED,tc2-today" >> "\$raw"
IN
...
if [ "${y_cnt:-0}" -eq 0 ] && [ "${t_cnt:-0}" -ge 1 ]; then pass=0; break; fi
