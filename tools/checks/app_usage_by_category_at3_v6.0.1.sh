#!/bin/sh
set -eu
OUT="evidence/v6.0/app_usage_by_category/at3.1.txt"
...
append_if_missing '!evidence/v6.0/app_usage_by_category'
append_if_missing '!evidence/v6.0/app_usage_by_category/.csv'
append_if_missing '!evidence/v6.0/app_usage_by_category/.txt'

git add .gitignore >/dev/null 2>&1 || true
git commit -m "AT3: whitelist evidence app_usage_by_category paths (v6.0.1)" >/dev/null 2>&1 || true
git rm -r --cached evidence >/dev/null 2>&1 || true

touch evidence/v6.0/app_usage_by_category/probe.txt
if git add evidence/v6.0/app_usage_by_category/probe.txt >/dev/null 2>&1; then
