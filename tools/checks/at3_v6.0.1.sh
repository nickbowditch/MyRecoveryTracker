#!/bin/bash
S=0
git rev-parse --is-inside-work-tree >/dev/null 2>&1 || { echo "AT-3 RESULT=FAIL (not a git repo)"; exit 2; }
...
[ $? -eq 0 ] && S=1
if [ "$S" -eq 0 ]; then echo "AT-3 RESULT=PASS"; exit 0; else echo "AT-3 RESULT=FAIL (.gitignore still ignores evidence)"; exit 1; fi
