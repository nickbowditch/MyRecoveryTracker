...
git commit -m "gitignore: whitelist evidence/v6.0/lnsu (AT3 v6.0.4)" >/dev/null 2>&1 || true

touch evidence/v6.0/lnsu/probe.txt
git add evidence/v6.0/lnsu/probe.txt >/dev/null 2>&1 || true

if git ls-files --error-unmatch evidence/v6.0/lnsu/probe.txt >/dev/null 2>&1; then
  echo "AT3 RESULT=PASS" | tee "$OUT"
  exit 0
fi

echo "AT3 RESULT=FAIL (probe not addable — higher-level ignore?)" | tee "$OUT"
exit 1
