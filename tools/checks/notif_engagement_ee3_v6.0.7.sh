...
OUT="evidence/v6.0/notification_engagement/ee3.7.txt"
...
($0 ~ p) && ($0 ~ /Engagement/) && ($0 ~ /(SUCCEEDED|SUCCESS|COMPLETED|FINISHED)/){print; exit}' || true)"
[ -n "$WM_OK" ] && { PASS=1; break; }

LOG_OK="$(adb logcat -d 2>/dev/null | grep -i "Engagement" | grep -i -E "enqueue|succeeded|success|completed|finished|done" || true)"
...
[ -n "$LOG_OK" ] && echo "$LOG_OK" || (adb logcat -d 2>/dev/null | grep -i -E 'TriggerReceiver|Engagement|WorkManager' || echo "[none]")
} | tee "$OUT" >/dev/null
