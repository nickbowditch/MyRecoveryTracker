...
OUT="evidence/v6.0/sleep/di3.19.txt"
...
TMPN="$(adb shell toybox date +%s%N 2>/dev/null | tr -d '\r' || adb shell date +%s 2>/dev/null | tr -d '\r' || echo $$)"
TSUM="files/.di3_sum.$TMPN.csv"
TDUR="files/.di3_dur.$TMPN.csv"

adb exec-out run-as "$PKG" sh -c '
set -eu
cp "'"$SUM"'" "'"$TSUM"'" 2>/dev/null || exit 1
cp "'"$DUR"'" "'"$TDUR"'" 2>/dev/null || exit 1
' >/dev/null 2>&1 || { echo "DI-3 RESULT=FAIL (snapshot failed)" | tee "$OUT"; exit 7; }

adb exec-out run-as "$PKG" tail -n +2 "$TSUM" 2>/dev/null | tr -d '\r' | awk -F',' '
function date_ok(d){ return d ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ }
# Allow HH:mm[:ss] where HH is 00–23, OR exactly 24:00 or 24:00:00
function time_ok(t){
return (t=="" ||
...
if(hrs!=""){ if(!(is_num(hrs) && (hrs+0)>=0 && (hrs+0)<=18.0)) { bad=1; exit } }
}
END{ exit bad?1:0 }' || { echo "DI-3 RESULT=FAIL (summary invalid)" | tee "$OUT"; exit 8; }

adb exec-out run-as "$PKG" tail -n +2 "$TDUR" 2>/dev/null | tr -d '\r' | awk -F',' '
function date_ok(d){ return d ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ }
function is_num(x){ return x ~ /^-?[0-9]+(\.[0-9]+)?$/ }
{
d=$1; hrs=$2
...
END{ exit bad?1:0 }' || { echo "DI-3 RESULT=FAIL (duration invalid)" | tee "$OUT"; exit 9; }
