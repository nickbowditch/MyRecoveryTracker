...
OUT="evidence/v6.0/notification_latency/tc2.19.txt"
mkdir -p "$(dirname "$OUT")"
...
#  - valid same-day pair on Y (counts toward Y)
#  - cross-midnight pair (Y posted, T clicked) should NOT count toward Y or T
...
before_y="$( (adb exec-out run-as "$PKG" cat "$DAILY" 2>/dev/null || true) | tr -d '\r' | getcount "$Y")"
before_t="$( (adb exec-out run-as "$PKG" cat "$DAILY" 2>/dev/null || true) | tr -d '\r' | getcount "$T")"
...
while :; do
  ...
  sleep 1
  after_y="$( (adb exec-out run-as "$PKG" cat "$DAILY" 2>/dev/null || true) | tr -d '\r' | getcount "$Y")"
  after_t="$( (adb exec-out run-as "$PKG" cat "$DAILY" 2>/dev/null || true) | tr -d '\r' | getcount "$T")"
  inc_y=$(( ${after_y:-0} - ${before_y:-0} ))
  inc_t=$(( ${after_t:-0} - ${before_t:-0} ))
  if [ "$inc_y" -ge 1 ] && [ "${inc_t:-0}" -eq 0 ]; then pass=0; break; fi
  [ "$(date +%s)" -ge "$deadline" ] && break
done

[ $pass -eq 0 ] && echo "TC2 RESULT=PASS" | tee "$OUT" || echo "TC2 RESULT=FAIL (inc_y=${inc_y:-0} inc_t=${inc_t:-0})" | tee "$OUT"
exit $pass
