...
set -e
mkdir -p files/_tc2
[ -f files/unlock_log.csv ] && mv files/unlock_log.csv files/_tc2/unlock_log.csv || true
...
adb exec-out run-as "$PKG" sh -c 'rm -f files/unlock_cursor files/unlock_cursor.txt state/unlock_cursor.txt files/unlock_rollup.state files/unlock_checkpoint.txt' >/dev/null 2>&1 || true
...
adb exec-out run-as "$PKG" sh -s <<'SH'
set -e
if [ -f files/_tc2/unlock_log.csv ]; then
  mv -f files/_tc2/unlock_log.csv files/unlock_log.csv
else
  rm -f files/unlock_log.csv
fi
