...
OUT="evidence/v6.0/_repo/unlocks_device_snap.2.txt"
set -euo pipefail
adb get-state >/dev/null 2>&1 || { echo "DIAG-SNAP RESULT=FAIL" | tee "$OUT"; exit 2; }
adb shell pm path "$PKG" >/dev/null 2>&1 || { echo "DIAG-SNAP RESULT=FAIL" | tee "$OUT"; exit 3; }
{
