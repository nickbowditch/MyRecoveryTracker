...
TARGET="$(printf '%s\n' "$DAILY_DATA" | awk -F, -v t="$TODAY" 'NR>1 && $1 ~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ && $1<t {print $1}' | sort | tail -n1)"
[ -n "$TARGET" ] || fail "(no closed day in daily)"
...
if [ "$N" -eq 0 ]; then
  ...
  [ $OKP -eq 1 ] || { echo "BAD $TARGET cnt=0 (percentiles not blank/0)" | tee "$OUT"; echo "AT2 RESULT=FAIL" | tee -a "$OUT"; exit 1; }
  {
