...
EXPECTED_WORKER="UnlockRollupWorker"
EXPECTED_REDCAP_FIELD="daily_unlocks"
VALUE_MIN=1
VALUE_MAX=500
OUT="evidence/v6.0/attention/unlock_row_healthcheck_v6_0.txt"

mkdir -p "$(dirname "$OUT")"
exec >"$OUT" 2>&1

echo "[INFO] Unlock Row Healthcheck v6.0"
echo "[INFO] PKG=$PKG"
echo "[INFO] CSV=$CSV"
echo "[INFO] EXPECTED_WORKER=$EXPECTED_WORKER"
echo "[INFO] EXPECTED_REDCAP_FIELD=$EXPECTED_REDCAP_FIELD"
echo "[INFO] VALUE_RANGE_UNLOCKS=${VALUE_MIN}-${VALUE_MAX}"
...
printf '%s' "$HEADER" | awk -F',' '
  ...
  {
    gsub(/^[ \t]+|[ \t]+$/,"",$1); gsub(/^[ \t]+|[ \t]+$/,"",$2);
    if (tolower($1)=="date" && tolower($2)=="unlocks") ok=1
  }
  END{ exit ok?0:1 }' && HEADER_OK="yes"
echo "header_ok: $HEADER_OK"
[ "$HEADER_OK" = "yes" ] || { echo "RESULT=FAIL - header mismatch (expected: date,unlocks)"; exit 1; }
...
while IFS= read -r v; do
  [ -z "$v" ] && continue
  val="$(printf '%s' "$v" | tr -d '[:space:]')"
  echo "$val" | grep -Eq '^[0-9]+$' || SANITY_OK="no"
  awk -v x="$val" -v lo="$VALUE_MIN" -v hi="$VALUE_MAX" 'BEGIN{exit !(x>=lo && x<=hi)}' || SANITY_OK="no"
done <<__EOVALS__
$TODAY_VALS
__EOVALS__
echo "today_values_unlocks: ${TODAY_VALS:-<none>}"
echo "sanity_ok: $SANITY_OK (${VALUE_MIN}..${VALUE_MAX} unlocks)"
