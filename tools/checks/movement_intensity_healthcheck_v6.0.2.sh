...
for chk in $CHECKS; do
  ...
  if run_check "$chk"; then
    RESULTS="${RESULTS}$(echo "$chk" | sed 's/$/: PASS/')\n"
    continue
  fi
  ...
  case "$chk" in
    movement_intensity_at1.sh)
      sleep 2
      run_check "$chk" || {
        RESULTS="${RESULTS}$(echo "$chk" | sed 's/$/: FAIL/')\n"
        continue
      }
      RESULTS="${RESULTS}$(echo "$chk" | sed 's/$/: PASS (retry)/')\n"
      ;;
    *)
      RESULTS="${RESULTS}$(echo "$chk" | sed 's/$/: FAIL/')\n"
      ;;
