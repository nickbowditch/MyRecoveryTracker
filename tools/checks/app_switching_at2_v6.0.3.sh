...
# Ensure daily header and drop today's row to force recompute
...
# Trigger in correct order and wait for AppSwitchingDailyWorker SUCCESS
...
saw_success=0
while :; do
if adb logcat -d 2>/dev/null | grep -qiE 'AppSwitchingDailyWorker.Worker result SUCCESS|AppSwitchingDailyWorker: AppSwitchingDaily'; then
saw_success=1
break
fi
...
# Rebuild from usage_events with the SAME logic as the worker
