...
[ "$st" = "device" ] || { echo "TC-2 RESULT=FAIL (no device)"; exit 2; }
...
for f in files/unlock_log.csv files/daily_unlocks.csv files/unlock_cursor.txt files/unlock_cursor state/unlock_cursor.txt files/unlock_rollup.state files/unlock_checkpoint.txt; do
  if [ -f "$f" ]; then mv "$f" "files/_tc2/$(basename "$f")"; fi
done
...
adb exec-out run-as "$PKG" sh -s <<'SH'
set -e
# restore backups (if present) and clean temp
for f in unlock_log.csv daily_unlocks.csv unlock_cursor.txt unlock_cursor unlock_rollup.state unlock_checkpoint.txt; do
  if [ -f "files/_tc2/$f" ]; then mv -f "files/_tc2/$f" "files/$f"; else
    # if original absent, remove test file to leave no residue
    [ "$f" = "unlock_log.csv" ] && rm -f "files/unlock_log.csv"
    [ "$f" = "daily_unlocks.csv" ] && rm -f "files/daily_unlocks.csv"
  fi
done
[ -d files/_tc2 ] && rmdir files/_tc2 2>/dev/null || true
SH
