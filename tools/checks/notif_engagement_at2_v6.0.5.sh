...
BEGIN{lc=0; oc=0}
NR==1{
for(i=1;i<=NF;i++){ n=$i; gsub(/\r/,"",n); if(n=="ts"||n=="timestamp") tsi=i; if(n=="event") ei=i }
next
}
{ t=$tsi; e=$ei; if(length(t)>=10 && substr(t,1,10)==d){ if(e=="POSTED"||e=="posted") lc++; if(e=="CLICKED"||e=="clicked") oc++ } }
END{print lc "," oc}
')"
[ -n "$rd" ] || rd="0,0"
...
printf "%b" "$RESULTS" | grep -q '^BAD ' && { printf "%b" "$RESULTS" | tee "$OUT"; echo "AT2 RESULT=FAIL" | tee -a "$OUT"; exit 1; }
