...
CSV="files/log_retention.csv"
EXPECTED_WORKER="LogRetentionWorker"
OUT="evidence/v6.0/system/log_retention_healthcheck_v6_0.txt"

mkdir -p "$(dirname "$OUT")"

echo "[INFO] Log Retention Healthcheck v6.0"
echo "[INFO] PKG=$PKG"
echo "[INFO] CSV=$CSV"
...
BODY="$(adb exec-out run-as "$PKG" tail -n +2 "$CSV" 2>/dev/null | tr -d '\r' | sed '/^[[:space:]]*$/d' || true)"
