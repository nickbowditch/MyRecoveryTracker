...
APP_DIR="$(adb shell run-as "$PKG" sh -c 'pwd' 2>/dev/null | tr -d '\r')" || true
[ -n "$APP_DIR" ] || fail "(run-as denied)"
IN="$APP_DIR/$IN_REL"
OUTCSV="$APP_DIR/$OUT_REL"
...
: > "'"$OUT_REL"'"
{
  ...
  echo "'"$TS_Y_P"',POSTED,tc2-a"
  echo "'"$TS_Y_C"',CLICKED,tc2-a"
  echo "'"$TS_T_P"',POSTED,tc2-b"
  echo "'"$TS_T_C"',CLICKED,tc2-b"
} > "'"$IN_REL"'"
