...
OUT="evidence/v6.0/sleep/at2.6.txt"
mkdir -p "$(dirname "$OUT")"
...
NR==FNR{
  if(FNR==1){for(i=1;i<=NF;i++){if($i=="date") di=i;if($i=="duration_hours") hi=i;if($i=="duration_minutes") mi=i} next}
  d=$di; v=""; from=""
  if(hi && $hi!=""){v=$hi; from="hr"} else if(mi && $mi!=""){v=$mi; from="min"}
  if(from=="hr" && isnum(v)) h[d]=v+0.0
  else if(from=="min" && isint(v)) h[d]=(v+0)/60.0
  next
}
FNR==1{for(i=1;i<=NF;i++){if($i=="date") d2=i;if($i=="hours") h2=i} next}
FNR>1{d=$d2; v=$h2; if(isnum(v)) g[d]=v+0.0}
END{
  bad=0
  for(d in h){a=h[d]; b=(d in g)?g[d]:0; if(abs(a-b)>0.05){print "MISMATCH," d "," a "," b; bad=1}}
  for(d in g){b=g[d]; a=(d in h)?h[d]:0; if(abs(a-b)>0.05){print "MISMATCH," d "," a "," b; bad=1}}
