...
T="${HC_TIMEOUT:-25}"
LOG="evidence/v6.0/unlocks/health_unlocks.19.txt"
export LC_ALL=C
...
recap() {
  grep -E 'RESULT[[:space:]]*[:=][[:space:]]*(PASS|FAIL)' "$1" | \
...
# order adjusted so DI-1 runs immediately after any migration fixture
