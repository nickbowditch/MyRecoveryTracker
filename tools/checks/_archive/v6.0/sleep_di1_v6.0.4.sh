...
OUT="evidence/v6.0/sleep/di1.4.txt"
...
LOCK="$(tr -d $'\r' < app/locks/daily_sleep_duration.header 2>/dev/null)"
[ -n "$LOCK" ] || { echo "DI-1 RESULT=FAIL (missing lock header)" | tee "$OUT"; exit 4; }

HEAD="$(adb exec-out run-as "$PKG" sh -c 'head -n 1 files/daily_sleep_duration.csv 2>/dev/null' | tr -d $'\r')"
[ -n "$HEAD" ] || { echo "DI-1 RESULT=FAIL (missing csv)" | tee "$OUT"; exit 5; }

if [ "$HEAD" = "$LOCK" ]; then
  echo "DI-1 RESULT=PASS" | tee "$OUT"; exit 0
else
  {
