...
h0="$(adb exec-out run-as "$PKG" sh -c 'cat "'"$CSV"'" 2>/dev/null | sha1sum 2>/dev/null | awk "{print \$1}"' || true)"
adb shell am broadcast -a "$ACT" -n "$CMP" >/dev/null 2>&1 || { echo "EE-3 RESULT=FAIL (broadcast1 failed)" | tee "$OUT"; exit 4; }
sleep 2
h1="$(adb exec-out run-as "$PKG" sh -c 'cat "'"$CSV"'" 2>/dev/null | sha1sum 2>/dev/null | awk "{print \$1}"' || true)"
adb shell am broadcast -a "$ACT" -n "$CMP" >/dev/null 2>&1 || { echo "EE-3 RESULT=FAIL (broadcast2 failed)" | tee "$OUT"; exit 5; }
sleep 2
h2="$(adb exec-out run-as "$PKG" sh -c 'cat "'"$CSV"'" 2>/dev/null | sha1sum 2>/dev/null | awk "{print \$1}"' || true)"
