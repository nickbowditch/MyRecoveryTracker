...
OUT="evidence/v6.0/sleep/di3.2.txt"
...
if [ "$dur_col" -eq 0 ]; then
  echo "DI-3 RESULT=FAIL (no duration column)" | tee "$OUT"; exit 5
fi

if [ "$dur_col" -gt 0 ]; then
  BAD="$(awk -F, -v c="$dur_col" '
    NR>1{
      v=$c
      if(v !~ /^-?[0-9]+$/){print $0; next}
      if(v<0 || v>1440){print $0; next}
      if(v==0 && ($2!="" || $3!="")){print $0; next}
      if($2!="" && $2 !~ /^[0-2][0-9]:[0-5][0-9](:[0-5][0-9])?$/){print $0; next}
      if($3!="" && $3 !~ /^[0-2][0-9]:[0-5][0-9](:[0-5][0-9])?$/){print $0; next}
    }' <<<"$CSV")"
else
  idx=$(( -dur_col ))
  BAD="$(awk -F, -v c="$idx" '
...
if [ -z "$BAD" ]; then
  echo "DI-3 RESULT=PASS" | tee "$OUT"; exit 0
else
  { echo "DI-3 RESULT=FAIL"; printf "%s\n" "$BAD"; } | tee "$OUT"; exit 1
fi
