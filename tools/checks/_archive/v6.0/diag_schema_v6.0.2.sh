...
OUT="evidence/v6.0/_repo/schema_sweep.2.txt"
set -euo pipefail
...
for p in "${MAP[@]}"; do
  ...
  LOCK="$(tr -d $'\r' < "$LOCKF" 2>/dev/null || true)"
  [ -n "$LOCK" ] || { bad=1; det="${det}MISSING_LOCK:$LOCKF"$'\n'; continue; }
  HEAD="$(adb exec-out run-as "$PKG" sh -c 'head -n 1 '"$CSVF"' 2>/dev/null' | tr -d $'\r' || true)"
...
if [ "$bad" -eq 0 ]; then
  echo "SCHEMA-SWEEP RESULT=PASS" | tee "$OUT"; exit 0
else
  { echo "SCHEMA-SWEEP RESULT=FAIL"; printf "%s" "$det"; } | tee "$OUT"; exit 1
fi
