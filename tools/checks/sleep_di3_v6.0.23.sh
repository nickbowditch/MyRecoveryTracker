...
OUT="evidence/v6.0/sleep/di3.23.txt"
...
# Self-regression guard: require :[0-5][0-9] and forbid the buggy :[0-5]0-9?
grep -q ':[0-5]\[0-9\]' "$0" || { echo "DI-3 RESULT=FAIL (self-check missing mm pattern)" | tee "$OUT"; exit 98; }
! grep -q ':\[0-5]0-9\?' "$0" || { echo "DI-3 RESULT=FAIL (self-check found buggy mm pattern)" | tee "$OUT"; exit 99; }
...
# Valid hh:mm[:ss] with mm and ss 00-59, allow 24:00[:00]
