...
fail() {
  ...
  { adb shell cmd deviceidle status 2>/dev/null || true; adb shell dumpsys deviceidle 2>/dev/null | head -n 60 || true; } | tr -d $'\r'
  ...
  echo "[DEBUG] --- CSV head ---"
  adb exec-out run-as "$PKG" sh -c 'head -n 20 "'"$CSV"'" 2>/dev/null | tr -d "\r"' || echo "<missing>"
...
echo "[INFO] EE3 — Runs under Doze/idle (fast, 15s deadline)"
