#!/bin/bash
OUT="evidence/v6.0/sleep/gv3.8.txt"
me="$(basename "$0")"
shopt -s nullglob
...
for f in "${SCRIPTS[@]}"; do
  b="$(basename "$f")"
  case "$b" in sleep_gv*) continue ;; esac
  [ "$b" = "$me" ] && continue
  s="${b%.sh}"
...
if [ "$miss" -eq 0 ]; then echo "GV-3 RESULT=PASS" | tee "$OUT"; exit 0; else { echo "GV-3 RESULT=FAIL"; printf "%s" "$det"; } | tee "$OUT"; exit 1; fi
