...
OUT="evidence/v6.0/lnsu/seed_daily_from_raw.5.txt"
...
D="$(adb shell toybox date -d "@$(( ${EPOCH:-0} - 86400 ))" +%F | tr -d '\r')"
N="$(adb shell toybox date -d "@$(( ${EPOCH:-0}        ))" +%F | tr -d '\r')"

adb exec-out run-as "$PKG" sh -c '
set -eu
daily="'"$CSV_DAILY"'"; raw="'"$CSV_RAW"'"; exp="'"$EXP"'"; d="'"$D"'"; n="'"$N"'"
mkdir -p files
if [ -f "$daily" ]; then
